...
bool parseArgs(int argc, char* argv[], std::string& r_name, std::string& start_date, std::string& end_date, int& num_threads, std::string& table_path, std::string& result_path) {
    ...
    for (int i = 1; i < argc; i += 2) {
        ...
        if (key == "--r_name") r_name = val;
        else if (key == "--start_date") start_date = val;
        else if (key == "--end_date") end_date = val;
        else if (key == "--threads") num_threads = std::stoi(val);
        else if (key == "--table_path") table_path = val;
        else if (key == "--result_path") result_path = val;
        else return false;
    }
